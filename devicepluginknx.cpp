...
void DevicePluginKnx::discoverDevices(DeviceDiscoveryInfo *info)
{
    if (info->deviceClassId() == knxNetIpServerDeviceClassId) {
...
void DevicePluginKnx::setupDevice(DeviceSetupInfo *info)
{
    qCDebug(dcKnx()) << "Setup device" << info->device()->name() << info->device()->params();
    ...
    if (info->device()->deviceClassId() == knxNetIpServerDeviceClassId) {
        QHostAddress remoteAddress = QHostAddress(info->device()->paramValue(knxNetIpServerDeviceAddressParamTypeId).toString());
        ...
        m_tunnels.insert(tunnel, info->device());
    }

    if (info->device()->deviceClassId() == knxTriggerDeviceClassId) {
        QHostAddress tunnelAddress =  QHostAddress(info->device()->paramValue(knxTriggerDeviceAddressParamTypeId).toString());
        ...
        info->device()->setParentId(m_tunnels.value(tunnel)->id());
    }

    if (info->device()->deviceClassId() == knxShutterDeviceClassId) {
        QHostAddress tunnelAddress =  QHostAddress(info->device()->paramValue(knxShutterDeviceAddressParamTypeId).toString());
        KnxTunnel *tunnel = nullptr;
        foreach (KnxTunnel *knxTunnel, m_tunnels.keys()) {
            if (knxTunnel->remoteAddress() == tunnelAddress) {
                tunnel = knxTunnel;
    ...
    if (info->device()->deviceClassId() == knxLightDeviceClassId) {
        QHostAddress tunnelAddress =  QHostAddress(info->device()->paramValue(knxLightDeviceAddressParamTypeId).toString());
        KnxTunnel *tunnel = nullptr;
        foreach (KnxTunnel *knxTunnel, m_tunnels.keys()) {
            if (knxTunnel->remoteAddress() == tunnelAddress) {
                tunnel = knxTunnel;
    ...
    if (info->device()->deviceClassId() == knxDimmableLightDeviceClassId) {
        QHostAddress tunnelAddress =  QHostAddress(info->device()->paramValue(knxDimmableLightDeviceAddressParamTypeId).toString());
        KnxTunnel *tunnel = nullptr;
        foreach (KnxTunnel *knxTunnel, m_tunnels.keys()) {
            if (knxTunnel->remoteAddress() == tunnelAddress) {
                tunnel = knxTunnel;
...
void DevicePluginKnx::executeAction(DeviceActionInfo *info)
{
...
void DevicePluginKnx::createGenericDevices(Device *parentDevice)
{
    ...
    if (!knxGenericSwitchDeviceDescriptors.isEmpty()) {
        ...
        emit autoDevicesAppeared(knxGenericSwitchDeviceDescriptors);
    }
    ...
    if (!knxGenericUpDownDeviceDescriptors.isEmpty()) {
        ...
        emit autoDevicesAppeared(knxGenericUpDownDeviceDescriptors);
    }
    ...
    if (!knxGenericScalingDeviceDescriptors.isEmpty()) {
        ...
        emit autoDevicesAppeared(knxGenericScalingDeviceDescriptors);
    }
    ...
    if (!knxGenericTemperatureSensorDeviceDescriptors.isEmpty()) {
        ...
        emit autoDevicesAppeared(knxGenericTemperatureSensorDeviceDescriptors);
    }
    ...
    if (!knxGenericLightSensorDeviceDescriptors.isEmpty()) {
        ...
        emit autoDevicesAppeared(knxGenericLightSensorDeviceDescriptors);
    }
    ...
    if (!knxGenericWindSpeedSensorDeviceDescriptors.isEmpty()) {
        ...
        emit autoDevicesAppeared(knxGenericWindSpeedSensorDeviceDescriptors);
    }
}
